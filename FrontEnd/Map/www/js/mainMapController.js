...
var mapMadule = angular.module('mainmap.controllers', ['ionic','ionic.rating']);
...
mapMadule.controller('MainMapCtrl',function ($scope,$state,$ionicPopover,$http){
  ...
  $scope.searchScene = function(searchSceneName,$event){
    $scope.title = "搜索结果";
    console.log("input:"+searchSceneName);
    $http({
      method: 'POST',
      url: "http://localhost:8080/server/scene/searching",
      params: {"scenetype":0,"keyword":searchSceneName,"searchtype":0},
      headers: {
        'Content-Type': 'application/json',
        'Accept' : 'application/json',
        'Access-Control-Allow-Origin': '*',
    ...
    //$scope.searchKey = searchSceneName;
  ...
  //$scope.mainSearch = function(searchSceneName,$event){
  ...
  //  //console.log(searchSceneName);
  //  $scope.popover.show($event);
  //  console.log("name:"+$scope.allItemList[0]);
  //
  //  console.log("searchInput:"+searchSceneName);
  //  //   $scope.popover.show($event);
  //
  //}
